...
class DrugCategoryController extends Controller
{
    public function all()
    {
        $categories= DrugCategory::where('pharmacy_id',Auth::user()->id)->latest()->where('status',1)->paginate(10);
        return response()->json($categories,200);
    }

    public function get(DrugCategory $category)
    {
    ...
    public function create(Request $request)
    {
        ...
        $drug_category = DrugCategory::create($request->all());
    ...
    public function update(Request $request)
    {
        ...
        $drug_category = DrugCategory::find($request->id);
